            ...
            {staedte.map((stadt, i) => (
              <span key={stadt.slug} className="inline-flex items-center">
                <Link
                  ...
                  className="text-sm text-white/70 hover:text-accent transition-colors no-underline"
                >
                  {stadt.name}
                </Link>
                {i < staedte.length - 1 && <span className="text-white/30 mx-1.5">·</span>}
              </span>
            ))}
          </div>
        </div>

        <div className="mt-8 pt-8 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-white/70">
          <p>&copy; {new Date().getFullYear()} Mizo Autohaus. Alle Rechte vorbehalten.</p>
          <div className="flex items-center gap-6">
            <Link to="/impressum" className="hover:text-white/70 transition-colors no-underline text-white/70">Impressum</Link>
